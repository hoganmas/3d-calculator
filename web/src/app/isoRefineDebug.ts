/** False-color iso refine: cyan = kept lo/mid, orange = finest remarch, magenta = iso intersection.
...
export function setIsoRefineDebugEnabled(on: boolean): void {
  ...
  console.log(
    `[iso-refine] debug ${on ? "ON — cyan = lo/mid, orange = fine edge, magenta = iso intersection" : "disabled"}`,
  );
}
...
export function initIsoRefineDebug(): void {
  ...
  if (isIsoRefineDebugEnabled()) {
    console.log(
      "[iso-refine] debug ON — cyan = lo/mid, orange = fine edge, magenta = iso intersection. " +
